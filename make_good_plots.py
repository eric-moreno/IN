...
def make_plots(outputDir, dataframes, tdf_train, savedirs=["Plots"], taggerNames=["IN"], eraText=r'2016 (13 TeV)'):
    ...
    def plot_rocs_with_DDT(dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname="", DDT_results = [[],[]]):
        f, ax = plt.subplots(figsize=(10, 10))
        colors = ['blue', 'navy', 'cornflowerblue', 'orange', 'orangered', 'orange']
        line_styles = ['-', '--', '-.' , ':', '--', '-']
        line_widths = [2.5, 2.5, 2.5, 3, 2.5, 2.5]
        print(len(dfs))
    ...
    def plot_jsd(dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname=""):
        ...
        ax.annotate(eraText, xy=(2.5e2, 1.1e7), fontname='Helvetica', ha='left',
                    bbox={'facecolor':'white', 'edgecolor':'white', 'alpha':0, 'pad':13}, annotation_clip=False)
        ax.annotate('$\mathbf{CMS}$', xy=(1.1, 1.1e7), fontname='Helvetica', fontsize=24, fontweight='bold', ha='left',
                    bbox={'facecolor':'white', 'edgecolor':'white', 'alpha':0, 'pad':13}, annotation_clip=False)
        ax.annotate('$Simulation\ Open\ Data$', xy=(2.3, 1.1e7), fontsize=18, fontstyle='italic', ha='left',
                    annotation_clip=False)
    ...
    def sculpting_multiple_taggers(tdf_array, names_array, siglab="Hbb", sculp_label='QCD', savedir="", sculpt_wp = 0.01):
        ...
        line_styles = ['--', '-.', ':', '--', '-.', '-']
        line_widths = [2.5, 2.5, 3, 2.5, 2.5, 2.5]
    ...
    plot_rocs_with_DDT(dfs=[cut(cutrho(frame)) for frame in dataframes],
              savedir=os.path.join(outputDir,'Plots'),
              names=taggerNames,
             sigs=[['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb']],
              bkgs=[['QCD'],['QCD'],['QCD'],['QCD'],['QCD'],['QCD']], DDT_results = [TPR_range, FPR_range])
    
   
    plot_jsd(dfs=[cut(cutrho(frame)) for frame in dataframes],
             savedir=os.path.join(outputDir,'Plots'),
             names=taggerNames,
             sigs=[['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb'],['Hbb']],
             bkgs=[['QCD'],['QCD'],['QCD'],['QCD'],['QCD'],['QCD'],['QCD'],['QCD']])
...
def main(args):
    ...
    df = pd.read_pickle('/nfshome/jduarte/DL4Jets/DeepJet/Train/eval_deepDoubleB_reference_80x_start94x/output.pkl')
    #df_dec_lambda1 = pd.read_pickle('/nfshome/jduarte/DL4Jets/DeepJet/Train/eval_deepDoubleB_reference_80x_start94x_kldiv_lambda1_batch4096/output.pkl')
    df_dec = pd.read_pickle('/nfshome/jduarte/DL4Jets/DeepJet/Train/eval_deepDoubleB_reference_80x_start94x_kldiv_lambda2_batch4096/output.pkl')
    #df_dec_lambda4 = pd.read_pickle('/nfshome/jduarte/DL4Jets/DeepJet/Train/eval_deepDoubleB_reference_80x_start94x_kldiv_lambda4_batch4096/output.pkl')
    #df_dec_lambda8 = pd.read_pickle('/nfshome/jduarte/DL4Jets/DeepJet/Train/eval_deepDoubleB_reference_80x_start94x_kldiv_lambda8_batch4096/output.pkl')
